...
from server.schemas.services import NotificationSuscribeBody
from server.schemas.user import UserSchema
from server.utils import protected_route, get_user, get_db

router = APIRouter(dependencies=[Depends(protected_route), Depends(Limiter("1/day"))])
...
def subscribe_notification(
    ...
    if not user_fcm_token:
        # Create FCMToken
        new_user_fcmtoken = FCMToken(user_id=user["id"], fcm_token=body.token)
